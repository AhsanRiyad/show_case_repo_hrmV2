<template>
  <div>
    <v-card>
      ...
      <template v-slot:prepend="{ item, open }">
        ...
        <v-icon v-if="item.icon == 'mdi-folder-open'">
          {{ open  ? 'mdi-folder-open' : 'mdi-folder' }}
        </v-icon>
        <v-icon v-else router :to="{ name: item.link }" >
          {{ item.icon }}
        </v-icon>
      </template>
      ...
      <template slot="label" slot-scope="{ item }">
        <router-link :to="{ name: item.link }" style="color: black; text-decoration: none;">
          {{ item.name }}
        </router-link>
      </template>
  ...
  <v-row justify="center">
    <v-expansion-panels flat active-class >
      ...
      :key="i"
      >
      <v-expansion-panel-header flat>  <div>
  ...
  <v-expansion-panel-content>
    <v-expansion-panels popout v-if=" !Array.isArray(item.item) ">
      {{ item.item }}
    </v-expansion-panels>

    <v-expansion-panels popout v-else>
      <v-expansion-panel 
      v-for="(n,i) in item.item"
      :key="i"
      >
...
<template v-slot:extension>
  ...
  <v-tab v-for="item in $store.getters.get_tabs_menu_list" :key="item.name" class="tab-hover" @click="()=>changeTab(item)" >
      ...
      return {
        ...
        },
        {
          ...
          children: [
          {
            name: 'static',
            children: [{
              name: 'logoaerfaffffffffffffffffffffffffffffffffffffffffffffffff.png',
              file: 'png',
            }],
          },
          {
    ...
    computed: {
      accessibleModules() {
        ...
        methods: {
          ...
          changeTab(tab) {
            console.log(tab);
            let left_side_bar_items =  this.$store.getters.get_state;
            // console.log(left_side_bar_items);
            // console.log(left_side_bar_items[tab.link].subModules);

            this.items_tree_view = [ ...left_side_bar_items[tab.link].subModules ]

            // console.log(left_side_bar_items.workStructureModule.subModules);
